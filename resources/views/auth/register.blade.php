            ...
            <div class="w-full rounded-lg shadow dark:border sm:max-w-md xl:p-0">
                <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                    ...
                    <form class="space-y-4 md:space-y-6" action="{{ route('register') }}" method="POST">
                        ...
                        <div>
                            <label for="name"
                                class="block mb-2 text-sm font-medium text-gray-100 dark:text-white">{{ __('Name') }}</label>
                            <input type="text" name="name" id="name" value="{{ old('name') }}"
                                ...
                                placeholder="{{ __('keywords.your_name') }}" required>
                            @error('name')
                        ...
                        <div>
                            <label for="password_confirmation"
                                class="block mb-2 text-sm font-medium text-gray-100 dark:text-white">{{ __('keywords.your_password_confirmation') }}</label>
                        ...
                        <p class="text-sm font-light text-gray-400 dark:text-gray-400">
                            {{ __('keywords.already_have_account') }}
                            <a href="{{ route('login') }}"
                                class="font-medium text-gray-500 hover:underline dark:text-gray-300">{{ __('keywords.sign_in') }}</a>
                        </p>
                    </form>
                </div>
            </div>
